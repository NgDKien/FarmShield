...
import { fetchQuarantineData } from '../services/faceDetectionApi'
    ...
    useEffect(() => {
        ...
        eventSource.onmessage = (event) => {
            ...
            if (data.status === 'update') {
                // Refetch data when update event is received
                const fetchData = async () => {
                    try {
                        const newData = await fetchQuarantineData();
                        setPerson(newData);
                    } catch (error) {
    ...
    return (
        <div className="font-main flex min-h-screen">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                {/* Wrapper căn giữa */}
                <div className="mt-[9px] w-full max-w-[1200px] mx-auto p-4 rounded-[20px] border border-[#D6DDEA] bg-white mb-8">
                    {/* Table header */}
                    <div className="grid grid-cols-[repeat(5,minmax(0,1fr))] text-2xl font-semibold text-black mb-2">
                        ...
                        <div className="text-center">Thay đồ</div>
                        <div className="text-center">Rửa tay</div>
                    </div>
                    ...
                    <div className="h-[300px] overflow-y-auto">
                        ...
                        {person.map((personData)=> (
                            <div key={personData.personId} className="table-row-wrapper">
                                <div className="grid grid-cols-[repeat(5,minmax(0,1fr))] items-center text-xl text-black mb-2">
                                    <div className="text-center break-words">{personData.name}</div>
                                    <div className="text-center break-words">{new Date(personData.entryLog[0]?.timestamp || Date.now()).toLocaleDateString('vi-VN')}</div>
                                    <div className="text-center break-words">{new Date(personData.entryLog[0]?.timestamp || Date.now()).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</div>
                                    <div className="text-center break-words">
                                        {personData.SanitizeLog?.clothChange ? (
                                    ...
                                    <div className="text-center break-words">
                                        {personData.SanitizeLog?.handWashing ? (
                                            <span className="text-green-600 font-bold">✓</span>
                                        ) : (
                                            <span className="text-red-600 font-bold">✗</span>
                                        )}
